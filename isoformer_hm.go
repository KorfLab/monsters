...
func isoforms(seq string, emin int, imin int, smax int, gen int, gff string, idn string) *isozone {
	var iso isozone
	seqlen := len(seq)
	var dons []int
	var accs []int
	...
	lenf := len(idnf)
	mut_pos,_ := strconv.Atoi(idnf[lenf-3])
	mut_type := idnf[lenf-2]
	
	if string(mut_type) == "d" {
		dons = append(dons, mut_pos)
	}
	if string(mut_type) == "a" {
		accs = append(accs, mut_pos)
	}
...
func main() {
	...
	iso := isoforms(seq, emin, imin, smax, gen, gff, idn)
